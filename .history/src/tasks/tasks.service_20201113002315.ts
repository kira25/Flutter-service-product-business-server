...
export class TasksService {
  ...
  async addTeamMembers(data: any, id) {
    ...
    return await this.taskModel.findOneAndUpdate(id, task, { new: true });
  }
  ...
  async deleteTask(id, memberId: string) {
    ...
    task.team.filter(team => team.memberId != memberId);

    const newTask = new this.taskModel(task);

    return await this.taskModel.findOneAndUpdate(id, newTask, { new: true });
  }
}
