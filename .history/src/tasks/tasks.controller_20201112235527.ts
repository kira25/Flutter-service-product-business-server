...
export class TasksController {
  ...
  async getTask(@Param('_id') _id: string): Promise<Task> {
    ...
    return await this.taskService.getTask(_id);
  }
  ...
  async updateTask(@Body() data: any, @Param('id') id: string): Promise<any> {
    return await this.taskService.addTeamMembers(data, id);
  }

  @Delete(':id/:memberId')
  deleteTask(@Param('id') id:string, @Param('memberId') memberId:string): Promise<Task> {
    return this.taskService.deleteTask(id,memberId);
  }
}
