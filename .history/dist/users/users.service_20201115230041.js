...
const nestjs_mailer_1 = require("@crowdlinker/nestjs-mailer");
const bcrypt = require("bcrypt");
const excel = require("xlsx");
const multer = require("multer");
let UsersService = class UsersService {
    constructor(userModel, jwtService, nodemailer) {
    ...
    async create(createUserDto) {
        try {
            ...
            if (!user) {
                ...
                createUserDto.password = bcryptjs_1.hashSync(createUserDto.password, salt);
                const payload = {
                ...
                return createUser.save().then(async (user) => {
                    console.log(user);
    ...
    async sendEmail(data) {
        ...
        const userUpdated = await this.userModel.findOneAndUpdate({
            _id: user._id,
        }, { resetPwdPing: code.toString(), resetPingUsed: false }, { new: true });
        const html = this.htmlPassword(userUpdated.firstname, userUpdated.resetPwdPing);
    ...
    async validatePasswordPin(data) {
        const email = data.email;
        ...
        if (user.resetPwdPing == passwordPin && user.resetPingUsed == false) {
            ...
            await this.userModel.findOneAndUpdate({
                _id: user.id,
            }, { resetPingUsed: true, resetPwdToken: resetPwdToken }, { new: true });
    ...
    async resetPassword(data) {
        ...
        if (user.resetPwdToken == passwordToken) {
            ...
            await this.userModel.findOneAndUpdate({
                _id: user.id,
            }, { resetPwdToken: undefined, password: newHashedPassword }, { new: true });
    ...
    uploadFile(data) {
        ...
        upload(data, function (err) {
            if (err) {
                return { ok: false, response: 'Strange error' };
            }
            if (!data.file) {
                return { ok: false, res: 'No file passed' };
            }
            if (data.file.originalname.split('.')[data.file.originalname.split('.').length - 1] === 'xlsx') {
                try {
                    const xls = excel.readFile(data.file.path);
                    ...
                    return { ok: false, response: 'Corrupted excel file' };
                }
            }
        });
    }
...
], UsersService.prototype, "uploadFile", null);
UsersService = __decorate([
    common_1.Injectable(),
    __param(0, mongoose_1.InjectModel('User')),
    __metadata("design:paramtypes", [mongoose_2.Model,
        jwt_1.JwtService,
        nestjs_mailer_1.Nodemailer])
], UsersService);
